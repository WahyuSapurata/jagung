            ...
            <div class="sidebar">
                ...
                <nav class="mt-2">
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                        ...
                        <li class="nav-item">
                            <a href="<?= base_url('admin/jagung') ?>" class="nav-link active">
            ...
            <section class="content">
                <div class="container-fluid">
                    ...
                    <div class="alert" style="margin-top: 70px;" role="alert">
                        <h3 class="m-0"><img src="<?= base_url('img/corn.png') ?>" width="27" class="nav-icon" alt=""> Jagung</h3>
                    </div>
                    <div class="card" style="margin-bottom: 70px;">
                        <div class="card-header">
                            <h5 class="card-title m-0">Tabel Data Jagung</h5>
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-hover text-center">
                                ...
                                <tbody>
                                    <?php $no = 1;
                                    foreach ($join_jagung as $jgn) { ?>
                                        <tr>
                                            <td><?= $no++ ?></td>
                                            <td><?= $jgn['judul'] ?></td>
                                            <td><?= $jgn['jenis_kategori'] ?></td>
                                            <td><?= buatRupiah($jgn['harga']) ?></td>
                                            <td><?= $jgn['tersedia'] ?></td>
                                            <td><?= $jgn['deskripsi'] ?></td>
                                            <td><img src="<?= base_url('uploads/jagung/' . $jgn['foto_jagung']) ?>" width="100" alt=""></td>
                                            <td>
                                                <form action="<?= base_url() ?>/admin/hapus_jagung/<?= $jgn['id_jagung'] ?>" method="post">
                                                    <button type="button" class="badge badge-warning" style="border: none; margin-right: 10px;" data-toggle="modal" data-target="#edit_modal<?= $jgn['id_jagung'] ?>">
                                        ...
                                        <div class="modal fade" id="edit_modal<?= $jgn['id_jagung'] ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title" id="myModalLabel">Edit Jagung</h4>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <form method="POST" enctype="multipart/form-data" action="<?= base_url('admin/edit_jagung/' . $jgn['id_jagung']) ?>">
                                                            <?= csrf_field(); ?>
                                                            <input type="hidden" name="lama" value="<?= $jgn['foto_jagung'] ?>">
                                                            <div class="form-group">
                                                                <label>Judul</label>
                                                                <input type="text" class="form-control" name="judul" value="<?= $jgn['judul'] ?>" required>
                                                            </div>
                                                            <div class="form-group">
                                                                ...
                                                                <select name="id_kategori" id="id_kategori" class="form-control">
                                                                    ...
                                                                    <?php foreach ($kategori as $kgt) : ?>
                                                                        <option value="<?= $kgt['id_kategori']; ?>" <?php if ($kgt['id_kategori'] == $jgn['id_kategori']) : ?> selected <?php endif ?>><?= $kgt['jenis_kategori']; ?></option>
                                                                    <?php endforeach ?>
                                                                </select>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Harga</label>
                                                                <input type="text" class="form-control" name="harga" value="<?= $jgn['harga'] ?>" required>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Tersedia</label>
                                                                <input type="text" class="form-control" name="tersedia" value="<?= $jgn['tersedia'] ?>" required>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Deskripsi Produk</label>
                                                                <textarea type="text" name="deskripsi" id="ckeditor" class="ckeditor" value="<?= $jgn['deskripsi'] ?>" required style="width: auto;"><?= $jgn['deskripsi'] ?></textarea>
                                                            </div>
                                                            <div class="form-group">
                                                                <label>Foto</label>
                                                                <input type="file" id="file_upload" class="custom" name="file_upload" value="<?= $jgn['foto_jagung'] ?>" onchange="gambar(this.value)">
                                                            </div>
                                                            <img src="<?= base_url('uploads/jagung/' . $jgn['foto_jagung']) ?>" id="foto" width="100" alt="" class="mb-3">
